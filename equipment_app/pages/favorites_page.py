...
class FavoritesPage(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(top, text="❤ Favori Ürünler", font=FONTS["subtitle"],
        ...
        ctk.CTkButton(top, text="🔄 Yenile", font=FONTS["body"],
                      ...
                      command=self.refresh).pack(side="right", padx=16, pady=9)
    ...
    def refresh(self):
        ...
        if not favs:
            ...
            empty_frame.grid(row=0, column=0, columnspan=3, pady=80)
            ctk.CTkLabel(empty_frame, text="💔", font=("Segoe UI", 60)).pack()
            ctk.CTkLabel(empty_frame, text="Henüz favoriniz yok", font=FONTS["subtitle"],
                         text_color=COLORS["accent"]).pack(pady=8)
            ctk.CTkLabel(empty_frame, text="Ürün detayında ❤ ikonuna basarak favorilere ekleyin",
                         font=FONTS["body"], text_color=COLORS["button"]).pack()
            ctk.CTkButton(empty_frame, text="📦 Ürünlere Git", font=FONTS["body"],
                          fg_color=COLORS["highlight"], hover_color=COLORS["highlight_hover"],
                          height=38, corner_radius=8,
                          command=lambda: self.navigate_callback("products")).pack(pady=16)
            return

        for idx, p in enumerate(favs):
            row, col = divmod(idx, 3)
            card = ctk.CTkFrame(self.scroll, fg_color=COLORS["bg_secondary"],
                                corner_radius=14)
            card.grid(row=row, column=col, padx=10, pady=10, sticky="nsew")

            icon = p.get("category_icon", "📦")
            ctk.CTkLabel(card, text=icon, font=("Segoe UI", 44)).pack(pady=(16, 2))
            ctk.CTkLabel(card, text=p.get("brand", ""), font=FONTS["small"],
                         text_color=COLORS["accent"]).pack()
            ctk.CTkLabel(card, text=p["name"], font=FONTS["heading"],
                         text_color=COLORS["text"], wraplength=220, justify="center").pack(padx=12, pady=(2, 4))
            ...
            ctk.CTkButton(btn_row, text="❌", font=("Segoe UI", 14),
                          fg_color=COLORS["button"], hover_color=COLORS["error"],
                          width=36, height=34, corner_radius=8,
                          command=lambda pid=p["id"]: self._remove_favorite(pid)).pack(side="left", padx=4)

...
class ComparePage(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        for side in ["a", "b"]:
            ...
            ctk.CTkLabel(
                f,
                text=f"Ürün {'1' if side == 'a' else '2'}",
                font=FONTS["body"],
    ...
    def _load_product_image(self, product_id, size=(140, 140)):
        """
    ...
    def _refresh_product_list(self):
        ...
        names = ["Seçiniz..."] + [
            f"{p.get('category_icon', '')}{p['brand']} - {p['name']}"
    ...
    def preselect_product(self, product_id):
        ...
        prod = next(
            (p for p in self._all_products if p["id"] == product_id),
            None
        )
        ...
        if value == "Seçiniz...":
            ...
            prod = next(
                (
                    ...
                    if f"{p.get('category_icon', '')}{p['brand']} - {p['name']}" == value
                ),
                None
            )
    ...
    def _render_compare(self):
        ...
        for col_idx, (prod, side_label) in enumerate([
            (self._prod_a, "Ürün 1"),
            (self._prod_b, "Ürün 2")
        ]):
            ...
            ctk.CTkLabel(
                col_frame,
                text=f"₺{float(prod['price']):,.0f}" if prod['price'] else "—",
            ...
            ctk.CTkButton(
                ...
                height=32,
                corner_radius=8,
                command=lambda pid=prod["id"]: self.navigate_callback("detail", pid)
            ...
            if prod.get("purchase_link"):
                ...
                ctk.CTkButton(
                    ...
                    height=32,
                    corner_radius=8,
                    command=lambda url=prod["purchase_link"]: webbrowser.open(url)
                ).pack(side="left", padx=4)
                            # AYIRICI
            ctk.CTkFrame(
                col_frame,
